...
The Hamiltonian for a neutron in a magnetic filed is
\begin{equation}
...
\end{equation}
In a presence of an electric field perpendicular to $\bm{B}$ and a harmonically oscillating nEDM
\begin{equation}
  H = - \gamma_\text{n} \mathbf{B} \cdot \mathbf{S} - \mathbf{d}_\text{n}^0 \sin (\omega_0 t) \cdot \mathbf{E}
\end{equation}
where we assume to be on the resonance with the oscillation.
In a rotating frame spinning around $\mathbf{B}$ with the frequency $\omega_0$ the field $\mathbf{B}$ vanishes~\cite{RamseyBook} and the Hamiltonian is
\begin{equation}
  H_\text{rot} = d_\text{n}^0 \mathbf{E} \cdot \mathbf{S}
\end{equation}
Following the analogy on a rotating transverse field we find the nutation frequency
\begin{equation}
  \omega_\text{nutation} = \frac{d_\text{n}^0}{\hbar}
\end{equation}
The angle for an interaction time $T$ is
\begin{equation}
  \theta = \omega_\text{nutation} T = \frac{d_\text{n}^0 E }{\hbar} \, T
\end{equation}
The neutron counts are proportional to the projection on the vertical axis
\begin{equation}
  N(\theta) = N_0 \alpha \sin \theta
\end{equation}
where $\alpha$ is the visibility parameter. For small angles we have
\begin{equation}
  N(d_n^0)  \approx \frac{N_0 \alpha T E}{\hbar} \, d_\text{n}^0
\end{equation}
The sensitivity is then, assuming it is dominated by the counting statistics $\sqrt{N}$ is
\begin{equation}
  \sigma_{d_n^0} = \frac{\hbar}{\sqrt{N} \alpha T E}
\end{equation}
