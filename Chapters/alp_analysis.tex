...
a pronounce shift corresponding to an nEDM of \SI{e-25}{\elementarycharge\centi\meter} was expected.
...
The sag caused vertical gradients in the magnetic field caused a shift in $R$.
...
\begin{figure}
  \centering
  \subfloat[An oscillating neutron electric dipole moment signal in the nEDM @ PSI apparatus.
  The colours indicate different electric field states: parallel to the magnetic field, antiparallel to it and zero]
  {\label{fig:axions_data_taking_one_run}
  \includegraphics[width=.34\linewidth]{gfx/axions/cycle-level_blinding_offset.png}}
  \quad
  \subfloat[An oscillating neutron electric dipole moment signal in the nEDM @ PSI apparatus across many runs.
  The colours indicate different electric field states: parallel to the magnetic field, antiparallel to it and zero.
  Different runs have different magnetic field gradients, which causes each run to have a different shift in $R = f_n / f_{Hg}$.]
  ...
  \note{Make a new figure, still hand-drawn, but nicer.
  Check the colouring scheme for the field polarity.
  Don't do subfigures---put everything one with an inset.}}\label{fig:oscillating_nEDM_in_R}
\end{figure}

A part of the measurement procedure was to deliberately work in a substantial vertical magnetic field gradient.
The gradient changed between sequences, causing large shifts in $R$, as illustrated in Fig.\,\ref{fig:oscillating_nEDM_in_R}.
...
\marginpar{The oscillating would become stricly constant only, if it would rotate. But a rotation is a sum of two linear oscillations.}
\begin{equation}
