...
The space of possible axion-induced signals is spanned by their amplitude (the strength of the coupling) and their frequency (the axion mass). The problem is, therefore, naturally set in the frequency domain. The analysis was performed in two steps two steps. First, the measurements were transformed from the time domain into the frequency one by evaluating the periodogram of the time series. In the second step the periodogram was checked for statistically significant signals.
...
\end{equation}
where $N$, the number of data points. Different normalisations may be used.
\marginpar{The noise bed is the periodogram of noise. If there is a signal, it is said to ,,rise out of the noise bed''.}
