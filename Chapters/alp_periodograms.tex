...
The structures in the $P(\omega_i)$ PDFs are caused solely by the nonuniformity in sampling times and in the sizes of the error--bars. In particular, we can identify a very large expected rise in power at \unit[0.05]{Hz}, corresponding exactly to the inverse spacing between the bunching (\unit[20]{s}) introduced in the example. Therefore, this frequency we expect a peak to appear in the periodogram even when there is no significant oscillation of this frequency. This is the reason why we should consider the most significant peak, rather than simply the highest. This fact makes the presented reasoning different from the one of~\cite{Scargle1982}.

Here introduce the CDF formalism. Let us define the power estimator at the $i$-th frequency $Z_i := P(\omega_i)$. We denote the cumulative density function (CDF) of the power estimator at the $i$-th frequency as $F_i(z)$. \note{Check what do we expect it do be!} We expect it to have a form\ldots We can estimate it from the MC simulations\ldots Then the $i$-th local p-value is directly $p_i = 1 - F_i\left( P^D(\omega_i) \right)$. The most significant peak is the one with the lowest local p-value.

Yet, we can see that in the example signal there are 15 peaks with p-values on a 2$\upsigma$ level, much more than we would expect. This is due to the so--called \emph{look--elsewhere effect}, best explained as follows: a one--in--a--thousand event in a system is not a surprise, if it occurs in one of a thousand different systems.

Another way of explaining this. The height of the most significant peak is a statistic itself, whose distribution we can estimate from the MC--generated null hypothesis signals. The distribution is depicted on the right--hand size in Fig.\,\ref{fig:basic_detection} in \note{orange}. We can see that even when no signal is present, the highest peak will most of the times have a height placing it between 2- and 3$\upsigma$ local significance bands.
...
\end{equation}
To claim a discovery, the \emph{false alarm probability} has to be at most in the range of $2.87\,\cdot\,10^{-7}$ (so--called 5--sigma) \cite{PDG2014}.
