...
\note{Make the figure for the example at the frequency of the signal}.
...
The discrete CDF estimate, or actually one-minus-CDF, so that the logarithmic $y$-scale can be leveraged to resolve the high-power tail, for the signal frequency \SI{0.17}{\hertz} is depicted in Fig.\,\ref{fig:P_best_signal_candidate} by green dots.

When the power in the time series is very high, as it is the case for the signal frequency \SI{0.17}{\hertz} in the example, obtaining the discrete CDF estimate would require unrealistically many Monte Carlo samples. An easy solution was to extrapolate the discrete CDF by a linear fit of the form
\begin{equation}
  F_i(z) = 1 - A_i \, e^{-B_i z} \ ,
\end{equation}
where $A_i$ and $B_i$ were free parameters, motivated by Eq.\,(\ref{eq:local_functional_form}). The result of the fit is depicted in Fig.\,\ref{fig:P_best_signal_candidate} with a blue line.
...
Instead, the parameter $N$ was obtained by fitting a curve described by Eq.\,(\ref{eq:global_CDF_functional_form}) to the discrete CDF estimate. CDFs corresponding to both values of $N$, the actual number of frequencies and the fitted one, are depicted in Fig.\,\ref{fig:P_look-elsewhere} and are almost overlapping.
