...
The central element of the active magnetic field compensation is the response matrix of the system, also called the SFC matrix \note{need to establish the nomenclature}. The SFC matrix of the nEDM@PSI SFC system and its properties were thoroughly discussed in Sec.\,\ref{fig:nEDM_SFC_matrix}.

There is a fundamental difference between the matrix of the nEDM@PSI's system and the next generation one. The nEDM@PSI's matrix was not a priori known. The system was constructed and the matrix was measured as the system's property. This could be considered as the reason that the matrix was ill-conditioned and the system required regularisation and PI control to be stable. Conversely, the new system takes the matrix into account already at the design phase. The coils are designed to produce magnetic field correspoding to expansion \note{Needs to be clear here!} and are orthogonal to one another situation like there with the weird combination will not happen Thanks to the coil design method, 
